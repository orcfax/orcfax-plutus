...
{-# INLINABLE oracleNFT #-}
oracleNFT :: Oracle -> AssetClass
oracleNFT oracle = AssetClass (oSymbol oracle, oracleTokenName)
...
mkOracleValidator oracle x r ctx =
  ...
  where
    ...
    inputHasToken :: Bool
    inputHasToken = assetClassValueOf (txOutValue ownInput) (oracleNFT oracle) == 1
    ...
    outputHasToken :: Bool
    outputHasToken = assetClassValueOf (txOutValue ownOutput) (oracleNFT oracle) == 1
...
updateOracle oracle x = do
    m <- findOracle oracle
    let c = Constraints.mustPayToTheScript x $ assetClassValue (oracleNFT oracle) 1
    case m of
        Nothing -> do
...
findOracle :: forall w s. Oracle -> Contract w s Text (Maybe (TxOutRef, (ChainIndexTxOut, ChainIndexTx), Integer))
findOracle oracle = do
    --utxos <- Map.filter f <$> utxosAt (oracleAddress oracle)
    utxos <- Map.filter f <$> utxosTxOutTxAt (oracleAddress oracle)
    return $ case Map.toList utxos of
        [(oref, (o, tx))] -> do
            -- x <- oracleValue (txOutTxOut o) $ \dh -> Map.lookup dh $ txData $ txOutTxTx o
            -- x <- oracleValue (toTxOut o) $ \dh -> Map.lookup dh $ txData $ tx
            x <- oracleValue (toTxOut o) $ \dh -> Map.lookup dh $ _citxData  tx
            return (oref, (o, tx), x)
        _           -> Nothing
  where
    f :: (ChainIndexTxOut, ChainIndexTx) -> Bool
    f (o, _) = assetClassValueOf (txOutValue $ toTxOut o) (oracleNFT oracle) == 1
...
runOracle op = do
  ...
  where
    ...
    go oracle = do
        handleError logError $ awaitPromise $ endpoint @"update" $ updateOracle oracle
